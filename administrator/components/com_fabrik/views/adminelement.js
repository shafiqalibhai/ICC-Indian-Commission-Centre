...
var fabrikAdminElement = new Class({
	initialize:function(plugins){
		...
		this.jsCounter = 0;
		this.jsactions = $A(['focus','blur','abort','click','change','dblclick','keydown','keypress','keyup','mouseup','mousedown','mouseover','select','load','unload']);
	...
	addJavascript: function(opt){
		if($type(opt) !== 'object'){
			opt = {code:'',action:'',js_e_event:'',js_e_trigger:'',js_e_condition:'',js_e_value:''};
		} 
		///$$$rob do we need this!? should it not be done elsewhere - HUGH!!! :D
		// $$$ hugh - you're asking ME???  Talk to those nice Joomla! devs who strip linebreaks out of text params!
		var rExp = /;/gi;
		var newString = new String (";\n")
		code = new Element('textarea', {'rows':8,'cols':40,'name':'js_code[]','class':'inputbox'}).appendText(opt.code.replace(rExp, newString));
		...
		var content = new Element('table', {'class':'adminform', 'id':'jsAction_'+this.jsCounter}).adopt(
			new Element('tbody', {'class':'adminform', 'id':'jsAction_'+this.jsCounter}).adopt([
				new Element('tr').adopt(new Element('td', {'colspan':2})),
				new Element('tr').adopt([new Element('td').appendText(this.translate.jsaction), new Element('td').adopt(action)]),
				new Element('tr').adopt([new Element('td').appendText(this.translate.code), new Element('td').adopt(code)]),
				new Element('tr').adopt(new Element('td', {'colspan':2}).appendText(this.translate.or)),
				new Element('tr').adopt(new Element('td', {'colspan':2}).adopt([
					evs, triggers, new Element('span').appendText(this.translate.wherethis), condition, new Element('input', {'name':'js_e_value[]','class':'inputbox', 'value':opt.js_e_value}) 
				])),
	...
	addValidation: function(pluginHTML, plugin){
		...
		//update the default template so that radio buttons get the correct validation count in their names
		str = str.replace(/\[0\]/g, '[' + this.validationCounter + ']');
		td.innerHTML = str;
		var display =  'block';
		
		var c = new Element('div', {'class':'validationContainer'}).adopt(
		new Element('table', {'class':'paramlist admintable','id':'validationAction_' + this.validationCounter, 'styles':{'display':display}}).adopt(
		new Element('tbody').adopt(
			[
				new Element('tr').adopt(
						[
							new Element('td', {'class':'paramlist_key'}).appendText(this.translate.action),
							new Element('td', {'class':'paramlist_value'}).adopt(
								this._makeSel('inputbox elementtype', 'params[validation-plugin][]', this.plugins, plugin)
							)
						]
					),
		...
		var activePlugin = $E(' #validationAction_' + this.validationCounter + ' .page-' + plugin);
		if(activePlugin){
			activePlugin.setStyle('display','block');	
		}
		
		//watch the drop down
		$E('#validationAction_' + this.validationCounter + ' .elementtype').addEvent('change', function(e){
			e = new Event(e);
			var id = $(e.target).up(3).id.replace('validationAction_', '');
			$$('#validationAction_' + id + ' .elementSettings').each(function(d){
				d.style.display = 'none'; 
			});
			var s = e.target.getValue();
			if(s != this.translate.please_select){
				$E('#validationAction_' + id + ' .page-' + s).style.display ='block'; 
			}
			e.stop();
		}.bind(this));
		this.watchDeleteValidation();
	...
	_makeSel: function(c, name, pairs, sel, sellabel){
		...
		var opts = [];
		opts.push(new Element('option', {'value':''}).appendText(sellabel) )
