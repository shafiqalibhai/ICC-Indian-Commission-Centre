var adminVisualizationChart = new Class({
	...
	updateFields: function(e){
		...
		if(table != ''){
			...
			var url = this.options.livesite + 'index.php?option=com_fabrik&format=raw&controller=plugin&task=pluginAjax&g=visualization&plugin=chart&method=ajax_fields&k=2&t=' + table;
			var myAjax = new Ajax(url, { method:this.options.ajaxmethod, 
